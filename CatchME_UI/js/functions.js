var version = "1.0.0-beta",
	...
	mode = getE("catchme-mode"),
	hBootmode = getE("hBootmode"),
	hBootversion = getE("hBootversion"),
	hChipid = getE("hChipid"),
	hSdk = getE("hSdkversion"),
	hBm, hBv, hCi, hSk, hUt;
...
function getHeaderData() {
	getResponse("sysinfo.json", function (responseText) {
		...
		hCi = res.chipid;
		hSk = res.sdkversion;
		hUt = res.uptime.slice(0, -3);

		hBootmode.innerHTML = "v." + hBm;
		hBootversion.innerHTML = "-m." + hBv;
		hChipid.innerHTML = hCi;
		hSdk.innerHTML = "v." + hSk;
		hUptime.innerHTML = new Date(hUt * 1000).toISOString().substr(11, 8);

		fadeIn();
	}, function () {
		notify("ERROR: Failed to load 'sysinfo.json'");
		fadeIn();
	});
}
...
function saveOpsMode() {
	indicate();
	showLoading();
	var url = "settingsSave.json";
	url += "?mode=" + mode.checked;

	getResponse(url, function (responseText) {
		if (responseText == "true") {
			indicate(true);
			notify("Reconnect and reload this page");
			restart(true);
		} else {
			indicate(false);
...
/* Switch CatchME operation mode */
if (mode != null) {
	mode.addEventListener('change', function () {
		this.checked = !this.checked;
		if (!this.checked) {
			if (confirm('Are you sure you want to change to Detector Mode ?') == true) {
				this.checked = true;
				saveOpsMode();
				autoReload();
			}
		} else {
			if (confirm('Are you sure you want to change to Attack Mode ?') == true) {
				this.checked = false;
				saveOpsMode();
				autoReload();
			}
...
function checkConnection() {
	setTimeout(function () { getResponse("ClientScanTime.json", function (responseText) { window.location.reload() }, function () { notify("Reconnect and reload this page"); checkConnection() }, 2000) }, 1300);
}
...
function restart(noIndication) {
	if (noIndication == true) {
		getResponse("restartESP.json?", function (responseText) {
			if (responseText !== "true") {
				notify("ERROR: Failed to restart CatchMe Device!");
				showLoading("hide");
			}
		}, function () {
			notify("ERROR: Failed to restart CatchMe Device!");
	...
	if (timeoutCallback == null) {
		timeoutCallback = function () {
			getResponse("ClientScanTime.json", function () {
				notify("ERROR: Didn't receive a response!");
			}, function () {
				notify("Reconnect and reload this page");
				autoReload();
			})
...
if (document.getElementsByTagName("footer")[0] != null) document.getElementsByTagName("footer")[0].innerHTML = "<div class=footer><ul><li><a href=#" + version + " target=blank_ style=color:#fff;font-weight:400><b>CatchME</b> " + version + "</a><li></ul></div><div class=sub-section-attribution><a href=./info#credits>Credits</a> <a href=https://github.com/HerwonoWr/CatchME target=blank_>Github</a> <a href=https://github.com/HerwonoWr/CatchME/Docs/MANUAL.md target=blank_>Guide</a> <i style='color:#5f5f5f; font-size:0.8rem;'>( CatchME is based upon the awesome work of Stefan Kremser <a style='margin:0;font-size:0.8rem;' href=https://github.com/spacehuhn/esp8266_deauther>esp8266 Deauther</a> & Sam Denty <a style='margin:0;font-size:0.8rem;' href=https://github.com/samdenty99/Wi-PWN>Wi-PWN</a> )</i></div>";

/* Dynamically add spinner */
if (document.getElementById("spinner-container") != null) document.getElementById("spinner-container").innerHTML = "<svg class=spinner viewBox='0 0 66 66'><circle class=path cx=33 cy=33 fill=none r=30 stroke-linecap=round stroke-width=6></circle></svg> <svg class=success-svg viewBox='0 0 1000 1000'><path d=M908.3,132.5L336.7,704.2l-245-245L10,540.8l326.7,326.7l81.7-81.7L990,214.2L908.3,132.5z /></svg> <svg class=failed-svg viewBox='0 0 19 19'><line stroke-width=2 x1=1 x2=18 y1=1 y2=18></line><line stroke-width=2 x1=18 x2=1 y1=1 y2=18></line></svg>"

/* Dynamically add SVG version of CatchME logo */
var svgLogo = '<svg style="width:30;height:30" viewBox="0 0 24 24"><path d="M12,16C12.56,16.84 13.31,17.53 14.2,18L12,20.2L9.8,18C10.69,17.53 11.45,16.84 12,16M17,11.2A2,2 0 0,0 15,13.2A2,2 0 0,0 17,15.2A2,2 0 0,0 19,13.2C19,12.09 18.1,11.2 17,11.2M7,11.2A2,2 0 0,0 5,13.2A2,2 0 0,0 7,15.2A2,2 0 0,0 9,13.2C9,12.09 8.1,11.2 7,11.2M17,8.7A4,4 0 0,1 21,12.7A4,4 0 0,1 17,16.7A4,4 0 0,1 13,12.7A4,4 0 0,1 17,8.7M7,8.7A4,4 0 0,1 11,12.7A4,4 0 0,1 7,16.7A4,4 0 0,1 3,12.7A4,4 0 0,1 7,8.7M2.24,1C4,4.7 2.73,7.46 1.55,10.2C1.19,11 1,11.83 1,12.7A6,6 0 0,0 7,18.7C7.21,18.69 7.42,18.68 7.63,18.65L10.59,21.61L12,23L13.41,21.61L16.37,18.65C16.58,18.68 16.79,18.69 17,18.7A6,6 0 0,0 23,12.7C23,11.83 22.81,11 22.45,10.2C21.27,7.46 20,4.7 21.76,1C19.12,3.06 15.36,4.69 12,4.7C8.64,4.69 4.88,3.06 2.24,1Z"></path></svg>'

if (document.getElementById("logo-img") != null) document.getElementById("logo-img").innerHTML = svgLogo;

/* Dynamically add reboot button */
if (!document.getElementById("catchme-setup")) document.body.insertAdjacentHTML('afterbegin', '<div class="reboot-container"><div class=reboot-inner><svg onclick="restart()" class=reboot viewBox="0 0 24 24"><path d="M17.65,6.35C16.2,4.9 14.21,4 12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20C15.73,20 18.84,17.45 19.73,14H17.65C16.83,16.33 14.61,18 12,18A6,6 0 0,1 6,12A6,6 0 0,1 12,6C13.66,6 15.14,6.69 16.22,7.78L13,11H20V4L17.65,6.35Z"/></svg></div><span class="tooltip">Reboot Device</span></div>');
...
Waves.attach('#links a', ['waves-light-blue']);
Waves.attach('.card-action .button', ['waves-blue']);
Waves.attach('.secondary', ['waves-blue']);
Waves.attach('button', ['waves-light']);
Waves.attach('.button', ['waves-light']);
Waves.attach('.reboot-inner');
Waves.init();

/* Add navbar shadow using scroll event listeners */
...
/* Get header informations data */
if (mode != null) getHeaderData();

/* Get operation mode */
if (mode != null) getOpsMode();