var table = document.getElementsByTagName('table')[0],
	...
	startCountdown,
	tableHeaderHTML = '<tr><th width="11%"></th><th>Name</th><th>Vendor</th><th>Client info</th><th>Pkts</th></tr>';
...
function getResults() {
	getResponse("ClientScanResults.json", function (responseText) {
		try {
			...
			notify("ERROR: Clear the client list");
			return;
		}
		...
		var tr = '';
		if (res.clients.length > 1) tableHeaderHTML = '<tr><th width="11%"><input type="checkbox" name="selectAll" id="selectAll" value="false" onclick="selAll()" ' + selectAllState + '><label class="checkbox" for="selectAll"></th><th>Name</th><th>Vendor</th><th>Client info</th><th>Pkts</th></tr>';
		tr += tableHeaderHTML;
		...
		for (var i = 0; i < res.clients.length; i++) {
			...
			if (res.clients[i].l >= 0) {
				tr += '<td class="darken-on-hover" onclick="setName(' + res.clients[i].i + ')"><span class="a b">' + escapeHTML(res.clients[i].n) + '</span><br></td>';
				tr += '<td>' + escapeHTML(res.clients[i].v) + '</td>';
			} else {
				tr += '<td class="darken-on-hover" onclick="setName(' + res.clients[i].i + ')"><span class="a light-6">save</span><br><span>Un-Name</span></td>';
				tr += '<td>' + escapeHTML(res.clients[i].v) + '</td>';
			}
			tr += '<td onclick="select(' + res.clients[i].i + ')">MAC: <b>' + res.clients[i].m + '</b><br>SSID: ' + escapeHTML(res.clients[i].a) + '</td>';
		...
		if (res.nameList.length != 0) {
			document.getElementById('saved-users').className = "card";
		}
		clientNames.innerHTML = res.nameList.length + " / 50";
		var tr = '<tr><th>Name</th><th>MAC Address</th><th>Actions</th></tr>';
		for (var i = 0; i < res.nameList.length; i++) {

			tr += '<tr>';
			tr += '<td>' + escapeHTML(res.nameList[i].n) + '</td>';
		...
		fadeIn();
		notify("Reconnect to WiFi network");
		checkConnection();
	}, 3000);
...
function scan() {
	...
	getResponse("ClientScan.json?time=" + scanTime, function (responseText) {
		if (responseText == "true") {
			...
			notify("No WiFi network(s) selected!");
			countdown(true);
		}
	});
}
...
function select(num) {
	...
	if ((time - previousCall) >= 80) {
		...
		getResponse("clientSelect.json?num=" + num, function (responseText) {
			...
			else notify("ERROR: Bad response 'clientSelect.json'");
		});
	}
}
...
function clearNameList() {
	if (confirm("Remove all saved users?") == true) {
		getResponse("clearNameList.json", function (responseText) {
			if (responseText == "true") getResults();
			else notify("ERROR: Bad response 'clearNameList.json'");
		});
	}
}
...
function addClient() {
	getResponse("addClient.json?mac=" + cMac.value + "&name=" + cName.value, function (responseText) {
		if (responseText == "true") {
			...
			macReset.value = '';
			nameReset.value = '';
		} else notify("ERROR: Bad response 'addClient.json'");
	});
}
...
function setName(id) {
	...
	if (newName != null) {
		getResponse("setName.json?id=" + id + "&name=" + newName, function (responseText) {
			if (responseText == "true") getResults();
			else notify("ERROR: Bad response 'editNameList.json'");
		});
	}
}
...
function deleteName(id) {
	getResponse("deleteName.json?num=" + id, function (responseText) {
		if (responseText == "true") getResults();
		else notify("ERROR: Bad response 'deleteName.json'");
	});
}
...
function add(id) {
	getResponse("addClientFromList.json?num=" + id, function (responseText) {
		if (responseText == "true") getResults();
		else notify("ERROR: Bad response 'addClientFromList.json'");
	});
}
