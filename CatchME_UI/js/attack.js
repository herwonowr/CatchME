...
function getResults() {
	getResponse("attackInfo.json", function (responseText) {
		try {
			...
			notify('ERROR: Failed to parse saved SSIDs, clearing...')
			resetSSID(true)
		}
		if (res.aps.length == 0) { document.getElementById("selectedNetworksDevices").className = "dn"; var aps = "" } else { document.getElementById("selectedNetworksDevices").className = "card-container"; var aps = "<tr><th>SSID</th><th>Action</th></tr>" }
		var clients = "<tr><th>MAC Address</th><th>Vendor</th></tr>";
		var tr = "<tr><th>Attack Mode</th><th>Status</th><th>Action</th></tr>";
		for (var i = 0; i < res.aps.length; i++) {
			aps += "<tr><td>" + res.aps[i] + "</td><td><button class='secondary clone' onclick=\"cloneSSID('" + escapeHTML(res.aps[i]) + "')\"><svg viewBox='0 0 1000 1000'xmlns=http://www.w3.org/2000/svg><path d='M700.5,10H165.9c-49.2,0-89.1,39.9-89.1,89.1v623.6h89.1V99.1h534.5V10L700.5,10z M834.1,188.2h-490c-49.2,0-89.1,39.9-89.1,89.1v623.6c0,49.2,39.9,89.1,89.1,89.1h490c49.2,0,89.1-39.9,89.1-89.1V277.3C923.2,228.1,883.3,188.2,834.1,188.2z M834.1,900.9h-490V277.3h490V900.9z'/></svg>clone</button></td></tr>";
		}
		...
		for (var i = 0; i < res.attacks.length; i++) {
			...
			tr += "</tr>";
			if (~res.attacks[i].name.indexOf('Probe-Request')) {
				if (res.randomMode == 1) {
					tr += "<tr class='selected'><td class='darken-on-hover' onclick='changeInterval()'>Random Attack <span class='light'><br>Interval: " + randomIntrvl + "s</span></td><td class='red'>running</td><td><button class='redBtn' id='randomBtn' onclick='random()'>stop</button></td></tr>"
				} else {
					tr += "<tr><td class='darken-on-hover' onclick='changeInterval()'>Random Attack <span class='light'><br>Interval: " + randomIntrvl + "s</span></td><td class='green'>ready</td><td><button id='randomBtn' class='secondary' onclick='random()'>start</button></td></tr>"
				}
			}
		}
		table.innerHTML = tr;
		...
		if (typeof res.ssid != 'undefined') {
			...
			var tr = "<tr><th>SSID</th><th>Action</th></tr>";
...
function startStop(num) {
	getResponse("attackStart.json?num=" + num, function (responseText) {
		...
		else notify("No WiFi network(s) selected!");
	});
}
...
function addSSID() {
	...
	if (isChecked) {
		...
		if (_ssidName.length > 0) {
			if (data.length >= 64) notify("ERROR: SSIDs list full", 2500);
			else {
